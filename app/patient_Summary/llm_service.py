...
def _build_language_instruction(input_language: Optional[str], output_language: Optional[str]) -> str:
    ...
    if out_lang_name:
        instructions.append(f"2. You MUST generate all dynamic section titles (e.g. 'Allergies' -> 'Alergias' in Spanish), names, and descriptions inside the JSON in this language: {out_lang_name}.")
    else:
        instructions.append("2. You MUST generate all dynamic section titles, names, and descriptions inside the JSON in the same language as the patient document/input information.")
        
    instructions.append("3. Keep the JSON structure keys (exactly 'patient_summary', 'title', 'items', 'name', 'description') in English. Do not translate the JSON keys themselves, only the generated values.")
