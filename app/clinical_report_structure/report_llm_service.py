...
Your ONLY task is to extract and document facts exactly as they appear in the provided inputs (live transcript, chat history, document texts). 

CRITICAL ANTI-HALLUCINATION RULES:
1. ZERO INVENTION (NO PRESCRIBING): DO NOT invent, suggest, or prescribe medications, dosages, treatments, follow-up dates, or specific ICD codes. If a medicine or plan is not EXPLICITLY spoken by the doctor in the transcript or written in the documents, it DOES NOT exist. Logical clinical deductions are STRICTLY FORBIDDEN.
...
def _build_prompt(
    ...
    if chat_history and len(chat_history) > 0:
        chat_str = json.dumps(chat_history, ensure_ascii=False, indent=2)
...
def _generate_report(prompt: str) -> dict:
    ...
    try:
        ...
        # ব্যাকএন্ডের সুবিধার জন্য আমরা চাইলে শুধু final_report রিটার্ন করতে পারি 
        # report_data = raw_json.get("final_report", raw_json)
...
async def generate_clinical_report_service(
    ...
    prompt = _build_prompt(live_transcript, chat_history, document_texts, example_structure)
